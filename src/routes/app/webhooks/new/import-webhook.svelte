<script lang="ts">
  import { createWebhook, type Webhook } from '$lib/database'
  import { getWebhookWithToken } from '$lib/discord'
  ...
  const handleWebhook = async (event: CustomEvent<Webhook>) => {
    ...
    try {
      ...
      await createWebhook({
        ...
        webhookAvatar: webhook.avatar || undefined,
  ...
  <div class="sm:mx-12 sm:my-4 sm:flex sm:flex-row sm:items-center sm:gap-12">
    ...
    <div>
      ...
      <p class="my-4 text-center sm:mx-0 sm:text-left">
        ...
        <a href="/app/editor">Editor</a> to send messages. You can also create
        <a href="/app/profiles">Profiles</a> to send messages with multiple identities.
      </p>
    </div>
  </div>
